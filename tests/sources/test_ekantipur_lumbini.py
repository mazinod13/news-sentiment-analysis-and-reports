"""Per-outlet test for eKantipur Lumbini. This file is yours alone -- no one else edits it.
...
def test_feed_parses(fixture_text):
    feed = feedparser.parse(fixture_text(f"{SOURCE_ID}_listing.html"))
    assert feed.entries, "feed produced no items"
    assert feed.entries[0].title
    assert feed.entries[0].link.startswith("http")


@pytest.mark.skip(reason="TODO: save ekantipur-lumbini_story.html and fill in the selector pack")
